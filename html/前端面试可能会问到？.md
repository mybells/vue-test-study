...
### 7. BFC（块级格式上下文）
  ...
  BFC文档规则：
    1. **在一个BFC中**，盒在垂直方向一个接一个地放置，从包含块的顶部开始。(块级元素特性)
    2. **在一个BFC中**，两个兄弟盒之间的垂直距离由margin属性决定。
    3. **同一个BFC中**，相邻块级盒之间的垂直外边距会合并。
    4. **同一个BFC中**，每个盒的左外边界（left outer edge）挨着包含块的左外边界（对于从右向左的格式化，右外边界挨着）。（这说明BFC中子元素不会超出他的包含块，而position为absolute的元素可以超出他的包含块边界）
    5. BFC的区域不会与float box重叠。(float box不在BFC内部)
    6. **同一个BFC中**，计算BFC的高度时，浮动元素也参与计算。一个bfc中包含一个bfc则失效
